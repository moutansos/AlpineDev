...
class AWSDyDB {
    constructor(node_env) {
        ...
        function handleCreateLoginTokenTable() {
            console.log("Sucessfully created the LoginTokens Table");
        }
    ...
    addToken(newToken, callback) {
        ...
        function handleDatabaseResponse(err, data) {
            ...
            callback(true, "Added token to database", newToken);
        }
    ...
    getTokenByUsername(username, callback) {
        function isEmptyObject(obj) {
            return !Object.keys(obj).length;
        }
        ...
        function handleAWSGet(err, data) {
            if(err) {
                var err = "Unable to get the token from the database. Error JSON: " + JSON.stringify(err);
                callback(err, null);
            } else if(isEmptyObject(data) || isEmptyObject(data.Item)) {
                var err = "No token found in database.";
                callback(err, null);
            } else {
                var token = new LoginToken(data.Item.userId,
                                           data.Item.expires,
                                           data.Item.token);
                callback(null, token);
            }
        ...
        function handleTableExist(exists) {
            if(!exists) {
                var error = "The token table does not exist!";
                callback(error, null);
                return;
            }
        ...
        this.tableExists(loginTokenTableParams.TableName, handleTableExist);
    }
